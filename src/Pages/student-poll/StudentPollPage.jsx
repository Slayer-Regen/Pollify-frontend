...
const StudentPollPage = () => {
  ...
  useEffect(() => {
    socket.on("pollCreated", (pollData) => {
      ...
      const correct = pollData.options.find((option) => option.correct === true);
      if (correct) {
        setCorrectAnswer(correct.text); // e.g., "distributed"
      } else {
        setCorrectAnswer(null); // fallback
      }
    });
  ...
  const getOptionClass = (option) => {
    if (submitted && correctAnswer === option.text) return "correct-answer";
    if (submitted && selectedOption === option.text) return "selected-submitted";
    if (!submitted && selectedOption === option.text) return "border option-border";
    return "";
  };
                ...
                <div className="card-body">
                  ...
                  <div className="list-group mt-4">
                    {pollOptions.map((option) => (
                      <div
                        key={option.id}
                        className={`list-group-item rounded m-1 ${getOptionClass(option)}
                        }`}
                        style={{
                          padding: "10px",
                          cursor:
